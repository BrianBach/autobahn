...
public final class IdcpManager {
	...
	private static boolean renewQuit;
	...
	public static void startSubscriptions() {
		...
		final int renewTime = DEFAULT_SUBSCRIPTION_TIME;
		Calendar termTime = Calendar.getInstance();
		termTime.add(Calendar.MINUTE, renewTime);
		...
		// thread for sending renew messages
		new Thread(new Runnable() {
			...
			public void run() {
				...
				while (!renewQuit) {
					...
					try {
						Thread.sleep(1000 * 60 * renewTime);
	...
	public static void stopSubscriptions() {
		...
		for (IdcpDomain dom : idcpDomains.values()) { 
			...
			try {
				IdcpNotifyClient notify = new IdcpNotifyClient(dom.getIdcpNotifyUrl());
				notify.unsubscribe(idcpNotifyUrl, "ALL", null);
				log.info("unsubscribed from " + dom.getIdcpNotifyUrl());
			} catch (Exception e)  {
				log.info("unsubscribed from " + dom.getIdcpNotifyUrl() + " failed - " + e.getMessage());
			}
		}
	}
