...
public class ProxyReceiver implements Closeable, Runnable {
        ...
        switch (op) {
            ...
            case Proxy.PROXY_NOTIFY_RESERVATION:
                ...
                try {
                    ...
                    ReservationInfo ri = (ReservationInfo)ois2.readObject();
                    ois2.close();
                    listener.Notify(ri);
                } catch (ClassNotFoundException e) { }
                break;
    ...
    public void run() {
        ...
        while (running) {
            ...
            try {
                System.out.println("ProxyReceiver - awaiting request");
                Socket sock = listen.accept();

                readProxy(sock);
                sock.close();
            } catch (IOException e) {
                System.out.println("ProxyReceiver stopped - " + e.getMessage());
            }
        }
    }
